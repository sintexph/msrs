...
@section('content')
    ...
    <form role="form" action="{{ route('bem_outsource_approval_deny', $service->id) }}" method="post">
    @csrf
        <button name="bem_approved" class="btn btn-default btn-block"> Deny</button>
    </form>

    <form role="form" action="{{ route('bem_outsource_approval_approve', $service->id) }}" method="post">    
    @csrf
