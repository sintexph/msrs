...
@section('content')
    ...
    <form role="form" action="{{ route('dept_request_approval_deny', $service->id) }}" method="post">
    @csrf
        <button name="dept_approved" class="btn btn-default btn-block"> Deny</button>
    </form>

    <form role="form" action="{{ route('dept_request_approval_approve', $service->id) }}" method="post">    
    @csrf
